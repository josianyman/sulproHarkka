...
class Sensor:
  ...
  def trigg(self):
    ...
    time.sleep(0.001)                         # Delay of 0.001 sec
  ...
  def measure(self):
    ...
    while GPIO.input(self.ECHO)==0:             # Check whether the ECHO is LOW
      pulse_start = time.time()                 # Save the last known time of LOW pulse

    pulse_end = time.time() 
    while GPIO.input(self.ECHO)==1:             # Check whether the ECHO is HIGH
      pulse_end = time.time()                   # Save the last known time of HIGH pulse 
